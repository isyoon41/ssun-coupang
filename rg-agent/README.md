...
- AI Provider 추상화(`src/lib/ai/get-provider.ts`) — `AI_PROVIDER` 환경변수로 전환합니다.
  - `mock`(기본값): `MockAiProvider`, 규칙 기반, API 키 불필요.
  - `google`: `GoogleAiProvider`(`src/lib/ai/google-provider.ts`), Gemini API(`GOOGLE_API_KEY`,
    `GOOGLE_AI_MODEL` 기본값 `gemini-2.0-flash`)를 실제로 호출해 11개 Agent 결과를 생성합니다.
  - `AiProvider` 인터페이스(`src/lib/ai/provider.ts`)를 구현하면 다른 LLM Provider로도 확장 가능합니다.
...
- OpenAI/Anthropic 등 추가 LLM Provider 연동: `src/lib/ai/provider.ts`의 `AiProvider` 구현
